...
class RNNLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # forget gate
        self.fg_w_x = nn.Linear(input_size, hidden_size)
        self.fg_w_h = nn.Linear(hidden_size, hidden_size)

        # input gate + new cell candidate
        self.ig_w_x = nn.Linear(input_size, hidden_size)
        self.ig_w_h = nn.Linear(hidden_size, hidden_size)
        self.nc_w_x = nn.Linear(input_size, hidden_size)
        self.nc_w_h = nn.Linear(hidden_size, hidden_size)

        # output gate
        self.og_w_x = nn.Linear(input_size, hidden_size)
        self.og_w_h = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, input, hidden, cell_state):
        # gates
        forget_gate = F.sigmoid(self.fg_w_x(input) + self.fg_w_h(hidden))
        input_gate = F.sigmoid(self.ig_w_x(input) + self.ig_w_h(hidden))
        cell_candidate = F.tanh(self.nc_w_x(input) + self.nc_w_h(hidden))
        output_gate = F.sigmoid(self.og_w_x(input) + self.og_w_h(hidden))
...
def train(category_tensor, line_tensor, rnn, criterion, optimizer):
  ...
  cell_state = rnn.init_cellstate()

  for i in range(line_tensor.size()[0]):
    output, hidden, cell_state = rnn(line_tensor[i], hidden, cell_state)
...
def main():
    ...
    rnn = RNNLSTM(letters_size, hidden_size, output_size)
